...
## helm/ subdir (its source lives in schema/helm/shared/ and its
...
## trees from schema/helm/shared/ to
...
HELM_TOOL_DIR := $(CURDIR)/schema/helm/tools/gen-versioned-helm
...
install: build
	...
	# live in schema/helm/), so it ships as-is.
