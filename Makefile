...
## all supported K8s versions under one PklProject. Preserves helm/ and
## the hand-written k8s.pkl back-compat shim.
#
# The master PklProject.deps.json is gitignored - resolve it so the
# pkl:reflect-driven discover.pkl can find @formae imports. The wipe
# spares helm/ (codegen source-of-truth + tools) and k8s.pkl (one-line
# re-export of v1.34/subresources.pkl that keeps `import "@k8s/k8s.pkl"`
# working post-schema-split).
generate-versioned-pkl-schemas:
	@pkl project resolve schema/pkl/main >/dev/null
	@find schema/pkl/generated -mindepth 1 -maxdepth 1 ! -name helm ! -name k8s.pkl -exec rm -rf {} + 2>/dev/null || true
...
#
# After copying the generated/ tree, prune helm/shared/, helm/tools/, and
# helm/README.md. They're codegen inputs, not user-facing schema, and
# formae's Extractor wildcard-imports every .pkl on the install path —
# the pre-rewrite helm/shared/*.pkl files use `@k8s/...` aliases that
