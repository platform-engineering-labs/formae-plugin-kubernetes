...
## helm/ subdir (its source lives in schema/pkl-helm/shared/ and its
## v<X.Y>/ trees are regenerated by `generate-versioned-helm-schemas`).
generate-versioned-pkl-schemas:
	...
	@# Wipe everything at schema/pkl's top level except helm/ (regenerated
	@# by generate-versioned-helm-schemas) and the build-time VERSION
	@# stamp, which isn't a codegen output.
	@find schema/pkl -mindepth 1 -maxdepth 1 ! -name helm ! -name VERSION -exec rm -rf {} + 2>/dev/null || true
...
## trees from schema/pkl-helm/shared/ to
## schema/pkl/helm/v<X.Y>/. Reads the K8s minor list from the
## sibling v<X.Y>/ directories produced by generate-versioned-pkl-schemas.
HELM_TOOL_DIR := $(CURDIR)/schema/pkl-helm/tools/gen-versioned-helm
generate-versioned-helm-schemas:
	@$(GO) run $(HELM_TOOL_DIR)
...
install: build
	...
	# schema/pkl/ holds only generated output (helm authoring sources
	# live in schema/pkl-helm/), so it ships as-is.
