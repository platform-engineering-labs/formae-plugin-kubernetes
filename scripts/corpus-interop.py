...
BOOTSTRAP = '''\
/// Generated by scripts/corpus-interop.py — a discoverable target, nothing else.
///
/// Three things are deliberately absent, and each omission was paid for:
///
/// No Release: it arrives via `formae extract`, which is what makes adoption
/// bind by the resource label formae itself chose.
///
/// No Namespace: `helm install --create-namespace` owns it, so formae does not.
/// K8S::Helm::Release declares `parent = "K8S::Core::Namespace"` with a
/// listParam (helm/Release.pkl:56-61), so discovery enumerates namespaces and
/// lists releases inside each. Declaring the namespace here made it managed, and
/// the release inside it then never appeared — 420s of polling and two forced
/// discovery passes found nothing. Leaving it unmanaged, the same release was
/// discovered in 8 seconds.
///
/// No Stack: formae rejects a forma that would create an empty one ("Stacks must
/// contain at least one resource"), and the only resource this run has is the
/// release, which does not exist in formae yet. The stack arrives with the
/// extracted forma instead.
...
def repair_extracted(path: Path, version_b: str, repo_url: str, stack: str = "") -> None:
    ...
    # Extract leaves the stack for a human to choose, as a commented-out label
    # inside an otherwise empty formae.Stack:
    #     local myStack = new formae.Stack {
    #       // Please provide a stack to bring the resources in this Forma ...
    #       // label = ""
    #     }
    # Applied as-is that is an empty stack, which formae rejects outright.
    if stack:
        text = re.sub(
            r'^(\s*)//\s*label\s*=\s*""',
            rf'\1label = "{stack}"',
...
def step_adopt(cell: Cell, version_b: str) -> Path:
    ...
    # Bind first at the live version, so the apply carries no change.
    #
    # reconcile, not patch, and only for this first apply. The stack does not
    # exist yet — the extracted forma is what creates it — and formae refuses a
    # patch against a stack it has never seen. The usual warning against
    # reconciling during adoption is about a stack that already holds the
    # namespace, where reconcile would treat it as absent and delete it, taking
    # the release with it. This stack holds nothing but the release, so there is
    # nothing for reconcile to remove. Every later apply is a patch.
    repair_extracted(adopted, cell.chart_version_a, "", stack=cell.stack)
    apply(cell.work, "reconcile", str(adopted))
...
def step_formae_upgrade(cell: Cell, adopted: Path, version_b: str) -> None:
    ...
    """
    repair_extracted(adopted, version_b, cell.chart.repo_url, stack=cell.stack)
...
def run_cell(chart: Chart, run_id: str, kube: str, formae_pkg: str, corpus: Path, keep: bool) -> Cell:
    ...
    try:
        ...
        print("\n2) formae bootstrap — a discoverable target")
        ...
        cell.ok("discoverable target created")
...
def self_check(corpus: Path) -> int:
    ...
    repair_extracted(tmp, "0.63.0", "https://k8s.ory.sh/helm/charts", stack="my-stack")
    fixed = tmp.read_text()
    check("stack placeholder filled", 'label = "my-stack"' in fixed, True)
    check("no commented label left", '// label = ""' not in fixed, True)
