...
def run(cmd: list[str], check: bool = True, timeout: int = 900) -> str:
...
def formae(*args: str, check: bool = True) -> str:
...
def apply(work: Path, mode: str, *extra: str, expect_failure: bool = False) -> str:
    ...
    for attempt in range(1, attempts + 1):
        out = formae("apply", "--mode", mode, "--yes", *extra, check=False)
...
BOOTSTRAP = '''\
/// Generated by scripts/corpus-interop.py — target, stack and namespace only.
///
/// Deliberately declares no Release. Discovery needs a target before it can see
/// anything, and the release itself arrives via `formae extract`, which is what
/// makes adoption bind correctly.
amends "@formae/forma.pkl"

import "@formae/formae.pkl"
import "@k8s/v{kube}/k8s.pkl" as k8s
import "@k8s/v{kube}/core/Namespace.pkl" as ns
...
def write_bootstrap(work: Path, chart: Chart, run_id: str, kube: str, formae_pkg: str) -> Path:
    ...
    forma.write_text(
        BOOTSTRAP.format(
            ...
            namespace=namespace_name(chart, run_id),
            chart=chart.name,
            trait=chart.trait,
        )
    )
...
def step_adopt(cell: Cell, version_b: str) -> Path:
    ...
    apply(cell.work, "patch", "--file", str(adopted))
...
def step_reconcile_absorbs(cell: Cell, adopted: Path) -> None:
    ...
    message = apply(cell.work, "reconcile", "--file", str(adopted), expect_failure=True)
    if not message:
...
def run_cell(chart: Chart, run_id: str, kube: str, formae_pkg: str, corpus: Path, keep: bool) -> Cell:
    ...
    try:
        ...
        preflight(cell)
        values = corpus / "values" / f"{chart.name}.yaml"
        ...
        step_helm_install(cell, version_a, values if values.exists() else None)

        print("\n2) formae bootstrap — target, stack, namespace")
        apply(work, "patch", "--file", str(work / "bootstrap.pkl"))
