...
EKS (commercial AWS only): trust the exact persisted subject with issuer
oidc.cloud.formae.ai and aud=sts.amazonaws.com for sts:AssumeRoleWithWebIdentity.
Broker allowlist: "sts.amazonaws.com". Set AwsOidcCredentials.roleArn to that
role; do not set an ambient profile. Provision an EKS access entry for this
ROLE ARN and suitable access policy/RBAC separately (API or API_AND_CONFIG_MAP
authentication mode). Role trust alone grants no Kubernetes access. Include
namespace create/delete plus workload grants for this example. Verify the
authenticated role and refresh beyond STS expiry on a real EKS fixture before
release; live EKS acceptance is still a release gate, not proved by this example.
