...
func (f *FlowSchema) Read(ctx context.Context, request *resource.ReadRequest) (*resource.ReadResult, error) {
	result, err := f.findByUID(ctx, request.NativeID)
	if err != nil {
...
func (f *FlowSchema) Delete(ctx context.Context, request *resource.DeleteRequest) (*resource.DeleteResult, error) {
	fs, err := f.findByUID(ctx, request.NativeID)
	if err != nil {
		if errors.IsNotFound(err) {
		...
		return nil, fmt.Errorf("failed to find flowschema: %w", err)
	}
	if fs == nil {
		return &resource.DeleteResult{
			ProgressResult: &resource.ProgressResult{
	...
	err = f.Client.FlowcontrolV1().FlowSchemas().Delete(ctx, fs.Name, metav1.DeleteOptions{})
	if err != nil {
		if errors.IsNotFound(err) {
			return &resource.DeleteResult{
				ProgressResult: &resource.ProgressResult{
...
// findByUID finds a FlowSchema by its UID.
func (f *FlowSchema) findByUID(ctx context.Context, uid string) (*flowcontrolv1.FlowSchema, error) {
	list, err := f.Client.FlowcontrolV1().FlowSchemas().List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
