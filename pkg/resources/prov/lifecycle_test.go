...
func TestRestoreEmptyObjects_InjectsEmptyMap(t *testing.T) {
	result := `{"spec":{"policyTypes":["Ingress"]}}`
	desired := `{"spec":{"podSelector":{},"policyTypes":["Ingress"]}}`
	got := restoreEmptyObjects(result, desired)

	var m map[string]any
	require.NoError(t, json.Unmarshal([]byte(got), &m))
	spec := m["spec"].(map[string]any)
	assert.Contains(t, spec, "podSelector")
	assert.Equal(t, map[string]any{}, spec["podSelector"])
}

func TestRestoreEmptyObjects_NoChangeWhenPresent(t *testing.T) {
	result := `{"spec":{"podSelector":{"matchLabels":{"app":"test"}},"policyTypes":["Ingress"]}}`
	desired := `{"spec":{"podSelector":{"matchLabels":{"app":"test"}},"policyTypes":["Ingress"]}}`
	got := restoreEmptyObjects(result, desired)
	assert.Equal(t, result, got)
}

func TestRestoreEmptyObjects_NoChangeWhenDesiredNotEmpty(t *testing.T) {
	result := `{"spec":{"policyTypes":["Ingress"]}}`
	desired := `{"spec":{"podSelector":{"matchLabels":{"app":"test"}},"policyTypes":["Ingress"]}}`
	got := restoreEmptyObjects(result, desired)
	assert.Equal(t, result, got)
...
func TestRestoreEmptyObjects_NestedEmptyObject(t *testing.T) {
	...
	var m map[string]any
	require.NoError(t, json.Unmarshal([]byte(got), &m))
	tmplSpec := m["spec"].(map[string]any)["template"].(map[string]any)["spec"].(map[string]any)
	assert.Contains(t, tmplSpec, "nodeSelector")
}

func TestRestoreEmptyObjects_EmptyInputs(t *testing.T) {
	assert.Equal(t, "", restoreEmptyObjects("", `{"spec":{}}`))
	assert.Equal(t, `{"spec":{}}`, restoreEmptyObjects(`{"spec":{}}`, ""))
}
