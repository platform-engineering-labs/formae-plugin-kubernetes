...
// NOTE: type-level (whole-kind) version support is NOT tracked here. The plugin
// derives "is this type available on the target's K8s version" directly from the
// installed per-version schema tree at runtime (see buildSupportMap in the main
// package), so it honors operator edits to the on-disk schema and needs no
// hand-maintained type map. This file remains the field-level gate registry.
